...
SELECT staff.first_name, staff.last_name, SUM(payment.amount)
    ...
    JOIN payment ON staff.staff_id = payment.staff_id
		AND payment.payment_date BETWEEN '2005-08-01' AND '2005-09-01'
...
SELECT cu.email, a.address
	FROM country co, city ci, address a, customer cu
...
SELECT s.store_id, SUM(p.amount)
	FROM store s, staff f, payment p
	WHERE s.store_id = f.store_id
		AND f.staff_id = p.staff_id
	GROUP BY s.store_id;
